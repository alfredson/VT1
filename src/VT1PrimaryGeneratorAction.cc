...
//       fGlobalTranslate = G4ThreeVector(0.0, 0.0, 12.1*cm);
//       fRandPosCarte = G4ThreeVector(posX, posY, posZ) + fGlobalTranslate;
//       G4cout <<"\nThe x position is " << G4BestUnit(posX, "Length") << G4endl;
//       G4cout <<"\nThe y position is " << G4BestUnit(posY, "Length") << G4endl;
//       G4cout <<"\nThe z position is " << G4BestUnit(posZ + 16.1*cm, "Length") << G4endl;
      
//       withinCryostat = TestIfPointIsInVolume(fRandPosCarte);
//       if (withinCryostat == 1)
// 	{
// 	  G4double ionCharge;
// 	  G4cout << "\nIsotope has been placed" << G4endl;
...
//   fMyNavigator->SetWorldVolume(fDetector->GetInnerVolume());
